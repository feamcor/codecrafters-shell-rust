use crate::commands::{
    dispatch_builtin, get_redirection, is_executable, run_executable, BuiltinAction,
};
use crate::jobs::JobManager;
use crate::parser::{
    ParsedCommand, COMMAND_CD, COMMAND_ECHO, COMMAND_EXIT, COMMAND_HISTORY, COMMAND_JOBS,
    COMMAND_PWD, COMMAND_TYPE,
};
...
pub fn execute_pipeline<H: rustyline::Helper, I: rustyline::history::History>(
    ...
    for (current_index, current_command) in pipeline.into_iter().enumerate() {
        ...
        let (stdout_builtin, stdout_external, new_previous_output) =
        ...
        let mut stderr_builtin =
            get_redirection(current_command.stderr.clone()).unwrap_or(Box::new(io::stderr()));
